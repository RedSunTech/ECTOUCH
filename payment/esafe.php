...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    $modules[$i]['config']  = array(
        array('name' => 'esafe_account',           'type' => 'text',   'value' => ''),
        array('name' => 'esafe_password', 'type' => 'text', 'value' => '')
    );
...
class esafe
{
    ...
    function get_code($order, $payment)
    {
        ...
        $form ='<form method="post" name="keyinorder" action="'.$url.'" target="_blank">';
        $form.='<input type="hidden" name="DueDate" value="'.date('Ymd',mktime(0,0,0,date("m"),date("d")+15,date("Y"))).'" />';
        $form.='<input type="hidden" name="UserNo" value="'.$order["user_id"].'" />';//<!-- 自訂給予下訂單的用戶之用戶編號 -->
        $res=$GLOBALS['db']->getAll("select * from ".$GLOBALS['ecs']->table('order_goods')." where order_id=".$order["order_id"]);
		foreach ($res as $key=>$row){
			$form.='<input type="hidden" name="ProductName'.($key+1).'" value="'.$row['goods_name'].'" />';
			$form.='<input type="hidden" name="ProductPrice'.($key+1).'" value="'.(int)$row['goods_price'].'" />';
			$form.='<input type="hidden" name="ProductQuantity'.($key+1).'" value="'.$row['goods_number'].'" />';
		}
		if ($order['shipping_fee']>0) {
		    $key++;
			$form.='<input type="hidden" name="ProductName'.($key+1).'" value="運費" />';
			$form.='<input type="hidden" name="ProductPrice'.($key+1).'" value="'.(int)$order['shipping_fee'].'" />';
			$form.='<input type="hidden" name="ProductQuantity'.($key+1).'" value="1" />';
		}
        $form.='<input type="hidden" name="web" value="'.$payment["esafe_account"].'" />';//商店代號
        ...
        $form.='<input type="hidden" name="note1" value="esafe" />';//備註1 maxlength="4000"
        $form.='<input type="hidden" name="note2" value="'.$order['log_id'].','.strtoupper(sha1($payment["esafe_account"].$payment["esafe_password"].$order['order_sn'].$order['log_id'].$order_amount)).'">';//備註2 maxlength="200"
        $form.='<input type="hidden" name="Card_Type" value="">';//交易類別
        $form.='<input type="hidden" name="ChkValue" value="'.strtoupper(sha1($payment["esafe_account"].$payment["esafe_password"].$order_amount)).'">';//備註2 maxlength="200"
    ...
    function respond()
    {
        ...
        $payment  = get_payment('esafe');
        $order_sn = $_GET['Td'];
        $order = explode(',',urldecode($_GET['note2']));
        $log_id = $order[0];
        if ($order[1] != strtoupper(sha1($payment["esafe_account"].$payment["esafe_password"].$order_sn.$log_id.intval($_GET["MN"])))) {
        	return false;
        }
        if ($_GET['ChkValue'] != strtoupper(sha1($payment["esafe_account"].$payment["esafe_password"].$_GET['buysafeno'].intval($_GET["MN"]).$_GET['errcode']))) {
            return false;
        }
        /* 检查支付的金额是否相符 */
